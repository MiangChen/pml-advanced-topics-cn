...
**图 20.1**：各类深度生成模型概览。这里 $x$ 是观测数据，$z$ 是潜编码（latent code），$x'$ 是来自模型的一个样本。AR 模型没有潜编码 $z$。对于扩散模型和流模型，$z$ 的大小与 $x$ 相同。对于 AR 模型，$x^d$ 是 $x$ 的第 $d$ 维。R 表示实值输出，0/1 表示二值输出。改编自 [[Wen21](../reference.md#Wen21)] 的图 1。

[^1]: 流模型定义了一个与 $x$ 同维的潜向量 $z$，尽管其内部的确定性计算可能使用比输入更大或更小的向量（参见例如 DenseFlow 论文 [[GGS21](../reference.md#GGS21)]）。
